...
1. 아래 git lfs 사이트에서 git lfs client를 다운로드 받아 설치해야 합니다.  
  https://git-lfs.github.com
2. LFS 설정  
클라이언트 설치후 한번만 실행해주시면 됩니다.  
`git lfs install`
3. lfs server url 설정 (repository 를 바꾸고자 하면)  
클라이언트 설치후 한번만 실행해주시면 됩니다.  

```
(local) $ git config add lfs.url https://github-lfs.{my-repository-host}
(global) $ git config --global lfs.url https://github-lfs.{my-repository-host}
```

> 기본적인 설정은 끝났습니다.  
> 하지만 위 설정만으로 100MB 이상의 파일이 자동으로 LFS로 관리되지는 않습니다.  
> LFS 로 버전관리를 하기 위해서는 각 repository별로 LFS로 관리할 파일들을 지정해줘야 합니다.  
> 아래 예시는 저장소에서 mp4확장자를 갖는 모든 파일(100MB 를 넘지 않더라도)들을 LFS로 관리하도록 하는 설정입니다.
...
> commit, push는 기존처럼 해주시면 됩니다.  
> push 이후 large file은 remote 에서 아래 형식으로 관리가 됩니다.  
> (참고, https://{my-repository-host}/sandbox/lfstest/blob/master/mobil2.mp4 )
> version https://git-lfs.github.com/spec/v1
> oid sha256:0b90ae6d8ac2abea0c7f525884d268b77954663720364a8c936e113779e2d1111
> size 806111111
...
> lfs.url 수정해서 lfs repository 바꾸기 (project 안에서만)

```console
$ git config lfs.url repository-url
```
